...
const preventClose = (state, action) =>
  action.type === 'closing' && state.openIndexes.length < 2
    ? {...action.changes, openIndexes: state.openIndexes}
    : action.changes

const single = (state, action) => {
  return action.type === 'opening'
    ? {...action.changes, openIndexes: action.changes.openIndexes.slice(-1)}
    : action.changes
}
